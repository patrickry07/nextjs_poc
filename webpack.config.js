...
let plugins = [
	...
	// new CleanWebpackPlugin(['templates/dist/*'], { root: path.resolve(__dirname , '..')}),
...
const buildProvider = (module_name, is_admin = false, src_template = './src/templates/empty_template.html') => {
...
const user_entry_points = {
	ask_expert:  ['./src/AskExpert.jsx'],
};
for (let module_name in user_entry_points) {
	plugins.push(buildProvider(module_name, false, user_entry_points[module_name][1]));
}
	...
	return {
		...
		module: {
			rules: [{
				...
				query: {
					helperDirs: [
						__dirname + '/app/js/_handlebars_helpers'
					]
				}
			},
			{
		...
		resolve: {
			alias: {
				...
				'react-hook-form': path.resolve(__dirname, 'node_modules', 'react-hook-form/dist/react-hook-form.ie11')
			},
			extensions: ['.js', '.jsx']
		},
